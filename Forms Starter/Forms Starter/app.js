  ...
  input.addEventListener("focus", () => {
    if (!input.value) {
      tl.fromTo(
        line,
        { attr: { d: start } },
        { attr: { d: end }, ease: "Power2.easeOut", duration: 1 }
      );
...
form.addEventListener("click", () => {
  containers.forEach((container) => {
    ...
    // we will do our validation here
    //name validation
...
// animating character
gsap.set("#eye", { transformOrigin: "center" }); // set transform origins
gsap.fromTo(
  "#eye",
  { scaleY: 1 },
  {
...
const tl3 = gsap.timeline({
  defaults: { duration: 0.75, ease: "Powe2.easeOut" },
});
...
button.addEventListener("click", (e) => {
  ...
  gsap.fromTo(
    "#hand",
    { rotation: 0, y: 0 },
    { rotation: -10, y: 2, ease: "elastic.out(3, 0.3)", duration: 2, delay: 1 }
  );
});
